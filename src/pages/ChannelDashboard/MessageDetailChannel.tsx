...
const DailyMessageChannel = (props: DialogInfoProps) => {
  ...
  if (params?.period === 'customrange' && params?.previousDate !== todayDate && params?.previousEndDate !== todayDate) {
    const startDate = moment(params?.date, 'YYYY-MM-DD')
    const endDate = moment(params?.endDate, 'YYYY-MM-DD')

    // console.log('souceId :'+paramsId?.sourceId)
    // console.log('platformId :'+params?.platformId)

    paramData = {
      campaign_id: params?.campaign || '',
      source: paramsId?.sourceId || params?.platformId || '',
      start_date: params?.date ? startDate.format('YYYY-MM-DD') : '',
      end_date: params?.endDate ? endDate.format('YYYY-MM-DD') : '',
